...
class VisionMamba(nn.Module):
    def __init__(self,
        ...
        # self.conv_f = nn.Conv2d(in_channels=3, out_channels=32, kernel_size=1, stride=1, padding=0)
        # self.conv_b = nn.Conv2d(in_channels=32, out_channels=3, kernel_size=1, stride=1, padding=0)
        ...
        self.mffa = MFFA(embed_dim=embed_dim, num_encoding_strategies=6)
        # pretrain parameters
        self.num_classes = num_classes
        self.d_model = self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        # self.fdconv = FDConv(
        #     in_channels=32,  # 输入通道数与图像通道数一致（默认3）
        #     out_channels=32,  # 输出通道数保持相同
    ...
    def forward_features(self, z, x, inference_params=None, if_random_cls_token_position=False,
        ...
        # x_k = self.conv_f(x)  # b, 32, 256, 256
        # z_k = self.conv_f(z)  # b, 32, 128, 128
        # x_fft = self.fft(x)  # b, 3, 256, 256
        # z_fft = self.fft(z)
        # x_b = self.conv_b(x_fd)
        # z_b = self.conv_b(z_fd)
        x = x1 = self.patch_embed(x)
        z = z1 = self.patch_embed(z)
        # x = self.patch_embed(x)  # x.shape = torch.Size([B, 3, 256, 256])  -> torch.Size([2, 256, 384])
        # z = self.patch_embed(z)
        Bx, Lx, Cx = x.shape
        Bz, Lz, Cz = z.shape
        x = x.view(Bx, 16, 16, Cx)
        z = z.view(Bz,8, 8, Cz)
        x = self.norm(x)
        z = self.norm(z)
        x = self.dynamic_filter16(x)    # B,16,16,384
        z = self.dynamic_filter8(z)      # B,8,8,384
        x = self.norm(x)
        z = self.norm(z)
        x = x.view(Bx,256,Cx) + x1
        z = z.view(Bz,64,Cz) + z1
        B, M, _ = x.shape
        ...
        if not self.if_bidirectional:  # True
            ...
            for layer in self.layers:
                ...
                count += 1
                if count % 4 == 0:
                    feat = hidden_states.permute(0, 2, 1)
        ...
        mffa_out = self.mffa(selected_columns[:6])
        hidden_states = mffa_out.permute(0, 2, 1)
